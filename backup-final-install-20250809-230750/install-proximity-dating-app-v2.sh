...
# Proximity Dating App - Clean Install Script v2
# This script cleans up unnecessary files and provides a fresh installation

set -e

echo "🚀 Proximity Dating App - Clean Install Script v2"
echo "================================================="
...
BACKUP_DIR="backup-clean-install-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
for dir in backup-*; do
    if [ -d "$dir" ]; then
        mv "$dir" "$BACKUP_DIR/" 2>/dev/null || rm -rf "$dir" 2>/dev/null || print_warning "Could not remove $dir"
...
unnecessary_files=(
    ...
    "install-proximity-dating-app.sh"
)

for pattern in "${unnecessary_files[@]}"; do
    if [ -f "$pattern" ]; then
        mv "$pattern" "$BACKUP_DIR/" 2>/dev/null || rm -f "$pattern" 2>/dev/null || print_warning "Could not remove $pattern"
        print_status "Moved $pattern to backup"
    elif [ -d "$pattern" ]; then
        mv "$pattern" "$BACKUP_DIR/" 2>/dev/null || rm -rf "$pattern" 2>/dev/null || print_warning "Could not remove $pattern"
        print_status "Moved $pattern to backup"
    fi
done
...
if [ -d ".next" ]; then
    if rm -rf .next 2>/dev/null; then
        ...
        print_warning "Could not remove .next directory (may be in use)"
        mv .next "$BACKUP_DIR/" 2>/dev/null || print_warning "Could not move .next to backup"
    fi
fi

if [ -d "socket-server/dist" ]; then
    if rm -rf socket-server/dist 2>/dev/null; then
        print_status "Removed socket-server/dist directory"
    else
        print_warning "Could not remove socket-server/dist directory (may be in use)"
        mv socket-server/dist "$BACKUP_DIR/" 2>/dev/null || print_warning "Could not move socket-server/dist to backup"
    fi
fi
...
if [ -d "node_modules" ]; then
    ...
    if rm -rf node_modules 2>/dev/null; then
        ...
        print_warning "Could not remove node_modules (may be in use)"
        mv node_modules "$BACKUP_DIR/" 2>/dev/null || print_warning "Could not move node_modules to backup"
    fi
fi
...
cd socket-server
if [ -d "node_modules" ]; then
    print_status "Removing existing socket-server node_modules..."
    if rm -rf node_modules 2>/dev/null; then
        print_success "Removed socket-server node_modules"
    else
        print_warning "Could not remove socket-server node_modules (may be in use)"
        mv node_modules "$BACKUP_DIR/" 2>/dev/null || print_warning "Could not move socket-server node_modules to backup"
    fi
fi
npm install
cd ..
...
print_complete "Step 5 completed: Application built"

# Step 6: Create startup scripts
print_step "Step 6: Creating startup scripts..."
...
print_complete "Step 6 completed: Startup scripts created"

# Step 7: Create environment files
print_step "Step 7: Setting up environment files..."
...
print_complete "Step 7 completed: Environment files setup"

# Step 8: Update documentation
print_step "Step 8: Updating documentation..."
...
1. Run the install script:
   ```bash
   bash install-proximity-dating-app-v2.sh
   ```
...
print_complete "Step 8 completed: Documentation updated"
